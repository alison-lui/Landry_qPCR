...
"""
THE DIFFERENCE WITH THIS FILE IS THAT I DIDN'T HAVE ENOUGH CF-LUV TO DO 
TRIPLICATES OF THE CONTROL OF CF-LUV PLUS TRITON

SO AFTER AVERAGING, WE NEED TO REPLACE THE AVERAGE OF CF-LUV PLUS TRITON
WITH JUST THE ACTUAL VALUES FROM B7
...
if AverageDatainTriplicates == True:
    ...
    " This is where I fix the data error - that the CF-LUV + Triton sample only has one replicate"
    # REPLACE TEMP_DATA COLUMN 6 with the unaveraged data
    temp_avg[:,6,:] = data[:,18,:]
    # REPLACE TEMP_STD COLUMN with zeros
    temp_std[:,6,:] = np.zeros(np.shape(temp_std[:,6,:]))
...
# PLOT 10 - NORMALIZED AND PHOTOBLEACH CORRECTED
...
for a in np.arange(0,len(headers_adds)): # for each figure
    ...
    for m in np.arange(0,len(headers_main)):
        ...
        X = [0] #CHANGE TO 1 IF YOU WANT TO SEE NORMALIZED BY TF
        for t in X:
            ...
            makeplot(data_norm_ctrl[:,:,:,t], stdev_norm[:,:,:,t], [HM], [ai], 0, fig, AX, [colors[m]], labels='main')
            AX.set_xlabel('Minutes')
            AX.set_ylabel('RFU')
...
# PLOT 11 - NORMALIZED AND PHOTOBLEACH CORRECTED
...
for m in np.arange(0,len(headers_main)): # for each concentration
    ...
    for i in np.arange(0,len(headers_adds)):
        ...
        # only label the first data point
        if i == 1:
            AX.plot(Time,data_norm_ctrl[:,plot_index,0,0], color=colors[m], label = HM)
        else:
            AX.plot(Time,data_norm_ctrl[:,plot_index,0,0], color=colors[m])
